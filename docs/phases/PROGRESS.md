...
- Final audit pass (1.0.0): four code audits against the master specification, every essential gap implemented (see docs/feature-audit.md): sidebars manager, page options box, page title depth, blog settings, layouts on every page type, popups post type, floating blocks, HTML block placements, coupon apply flow, analytics events, wishlist and compare fixes, maintenance and white label depth, portfolio templates, accessibility follow ups.
