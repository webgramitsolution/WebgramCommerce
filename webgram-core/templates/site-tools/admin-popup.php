...
<table class="form-table wgc-popup-settings" role="presentation">
	...
	<tr><th scope="row"><?php esc_html_e( 'Content', 'webgram-core' ); ?></th><td>
		...
		<select name="<?php echo esc_attr( $wgc_field( 'block' ) ); ?>"><option value="0"><?php esc_html_e( 'Select a block', 'webgram-core' ); ?></option>
		<?php
		foreach ( (array) $args['blocks'] as $wgc_id => $wgc_title ) :
?>
<option value="<?php echo (int) $wgc_id; ?>" <?php selected( (int) $wgc_s['block'], (int) $wgc_id ); ?>><?php echo esc_html( (string) $wgc_title ); ?></option><?php endforeach; ?></select>
	</td></tr>
	...
	<tr><th scope="row"><?php esc_html_e( 'Devices', 'webgram-core' ); ?></th><td>
	<?php
	foreach ( $wgc_devs as $wgc_k => $wgc_l ) :
?>
<label class="wgc-inline"><input type="checkbox" name="<?php echo esc_attr( $wgc_field( 'devices' ) ); ?>[]" value="<?php echo esc_attr( $wgc_k ); ?>" <?php checked( in_array( $wgc_k, $wgc_s['devices'], true ) ); ?>> <?php echo esc_html( $wgc_l ); ?></label> <?php endforeach; ?></td></tr>
	<tr><th scope="row"><?php esc_html_e( 'Show on', 'webgram-core' ); ?></th><td>
	<?php
	foreach ( $wgc_pages as $wgc_k => $wgc_l ) :
?>
<label class="wgc-inline"><input type="checkbox" name="<?php echo esc_attr( $wgc_field( 'pages' ) ); ?>[]" value="<?php echo esc_attr( $wgc_k ); ?>" <?php checked( in_array( $wgc_k, $wgc_s['pages'], true ) ); ?>> <?php echo esc_html( $wgc_l ); ?></label><br><?php endforeach; ?>
